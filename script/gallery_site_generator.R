  ...
  render <- function(input_file,
    ...
    sapply(files, function(x) {
      ...
      if (tools::file_ext(x) == "json") {
        meta <- jsonlite::read_json(x)
        output_file <- file_with_ext(basename(x), "html")
        if (!quiet) message("\nRendering to : ", output_file)
        x <- file_with_ext(sprintf(".tmp_%s", basename(x)), "Rmd")
        writeLines(
          glue::glue_data(
            meta,
            paste(readLines(file.path(config$layout, "_template.Rmd")), collapse = "\n"),
            .open = "{{", .close = "}}"
          ),
          x
        )
...
environment(gallery_site_generator) <- environment(rmarkdown::default_site_generator)
